...
const AdminWithdrawal = ({ logOut })=>{
    ...
    const processWithdrawal = ()=>{
        setFormLoading(true)
        fetch(`${process.env.REACT_APP_API_URL}/user/withdraw/${selectedDetails.withdrawal_id}`,{
            method: "put",
        ...
        .then((data)=>{
            ...
            if(data.success){
                showNotification("success", "Marked As Seen")
                setPage("All")
            }else{
                ...
                return(
                    <section className="transaction">
                        <div className="overlay">
                            <div className="first">
                                <button className="button" onClick = {()=>{setPage("All")}} style = {{"background": "#2aba66", "color": "#fff"}}>ALL</button>
                                <button className="button" onClick = {()=>{setPage("Seen")}}>SEEN</button>
                                <button className="button" onClick = {()=>{setPage("NotSeen")}}>NOT SEEN</button>
                            </div>
                                    ...
                                    <tbody>
                                        {withdrawals.map((withdrawal, i)=>(
                                            <tr className="tr" key = {i}>
                                                <td className="td1">{`${withdrawal.date.substring(0, withdrawal.date.indexOf("T"))}   ${withdrawal.date.substring(withdrawal.date.indexOf("T")+1, withdrawal.date.lastIndexOf(":"))}`}</td>
            ...
            case "Seen":
                return(
                    <section className="transaction">
                        <div className="overlay">
                            <div className="first">
                                <button className="button" onClick = {()=>{setPage("All")}}>ALL</button>
                                <button className="button" onClick = {()=>{setPage("Seen")}} style = {{"background": "#2aba66", "color": "#fff"}}>SEEN</button>
                                <button className="button" onClick = {()=>{setPage("NotSeen")}}>NOT SEEN</button>
                            </div>
                            <div className="overflow">
                                <table className="table">
                    ...
                    <section className="transaction">
                        <div className="overlay">
                            <div className="first">
                                <button className="button" onClick = {()=>{setPage("All")}}>ALL</button>
                                <button className="button" onClick = {()=>{setPage("Seen")}}>SEEN</button>
                                <button className="button" onClick = {()=>{setPage("NotSeen")}} style = {{"background": "#2aba66", "color": "#fff"}}>NOT SEEN</button>
                            </div>
                            <div className="overflow">
                                <table className="table">
                                    <thead>
                    ...
                    <div className="UserDetails">
                        ...
                        <section className="user">
                            ...
                            <div className="text">
                                ...
                                <h1 className="h1-tag">{selectedDetails.processed? "True": "False"}</h1>
                            </div>
                            <section className="daily">
                                {!selectedDetails.processed && <button className="user-button" onClick={processWithdrawal}>{formLoading? "....": "Mark As Seen"}</button>}
                            </section>
                        </section>
                    </div>
                )
