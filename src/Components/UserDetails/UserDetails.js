...
const UserDetails = ({ selectedUser, onNotify, setPage })=>{
    ...
    const onActivateAccount = (formData)=>{
		...
		.then(data=> {
            ...
            if(data.success){
                ...
                onNotify("error", "Unable to activate Account")
            }
		})
        .catch(err=>{
            console.log(err);
    ...
    const onDeactivateAccount = ()=>{
        setLoading(true)
        let deactivate = window.confirm("Are you sure you want to deactivate this account")
        if(deactivate){
            ...
            .then(data=> {
                ...
                if(data.success){
                    ...
                    onNotify("error", "Unable to deactivate Account")
                }
            })
            .catch(err=>{
                console.log(err);
    ...
    const addBonus = (formData)=>{
		...
		.then(data=> {
            ...
            if(data.success){
                ...
                onNotify("error", "Unable to add Bonus")
            }
		})
        .catch(err=>{
            console.log(err);
    ...
    const issueCard = (formData)=>{
        ...
        if(cardList.indexOf(card.type) !== -1){
            setLoading(false)
            onNotify("error", "This user already has an active card.")
        }else{
            ...
            .then(data=> {
                ...
                if(data.success){
                    ...
                    onNotify("error", "Unable to Issue Card")
                }
            })
            .catch(err=>{
                console.log(err);
                ...
                return(
                    <div className="activecard-wrapper">
                        ...
                        <section className="deactive">
                            <button className="user-button" onClick = {deleteCard}>{loading? "....": "Delete Card"}</button>
                        </section>
                    </div>
                )
            case "CLASSIC":
    ...
    useEffect(()=>{
        Promise.all([
            fetch(`${process.env.REACT_APP_API_URL}/user/transactions/${selectedUser.email}`),
            fetch(`${process.env.REACT_APP_API_URL}/user/card/${selectedUser.email}`)
          ])
            .then(async (result) => {
            ...
            <section className="board">
                {
                    ...
                    <> 
                        <section className="user-activecard"> 
                            {displayActiveCard()}
                        </section>
                        <section className="transaction" style={{height: "100%"}}>
                        ...
                        <section className="bonus">
                            ...
                            <form className="amount" onSubmit = {handleSubmit(addBonus)}>
                                ...
                                /><br/>
                                {errors.bonus && <h6 className = 'vError'>{errors.bonus.message}</h6>}
                                <input 
                                    className="input" 
                                    type="text" 
                                    placeholder="details"
                                ...
                                /><br/>
                                {errors.details && <h6 className = 'vError'>{errors.details.message}</h6>}
                                <input className="user-button" type="submit" value={loading? "....": "Add Bonus"}/>
                            </form>
                        </section>
                        <section className="bonus">
                            ...
                            <form className="amount" onSubmit = {handleSubmit2(issueCard)}>
                                ...
                                {errors2.card && <h6 className = 'vError'>{errors2.card.message}</h6>}
                                <input className="user-button" type="submit" value={loading? "....": "Issue card"}/>
                            </form>
                        </section>
                        <section className="deactive">
                            <p className="p-tag">Deactivate Account</p>
                            <button className="user-button" onClick = {onDeactivateAccount}>{loading? "....": "Deactive Account"}</button>
                        </section>
                    </>
                    :
                    <section className="daily">
                        <p className="p-tag">Set daily returns</p>
                        <form className="daily-input" onSubmit = {handleSubmit(onActivateAccount)}>
                            <input
                                type="number"
                                className="input"
                                name="daily_returns"
                                placeholder="Amount"
                                {...register('daily_returns', { required: 'Enter daily returns' })}
                            /><br/>
                            {errors.daily_returns && <h6 className = 'vError'>{errors.daily_returns.message}</h6>}
                            <input className="user-button" type="submit" value={loading? "....": "Activate Account"}/>
                        </form>
                    </section>
                }
